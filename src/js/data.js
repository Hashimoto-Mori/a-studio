...
function recording_csv_to_obj(t) {
  ...
  // 转换为对象
  for (let row of csv)
    window.gVariables.recording_list.push(convert_recording(row));
}
...
function song_csv_to_obj(t) {
  ...
  let csv = parse(t, { columns: true });
  // 转换为对象
  window.gVariables.song_list.splice(0, window.gVariables.song_list.length);
  for (let row of csv) window.gVariables.song_list.push(convert_song(row));
  // 按时间降序
  window.gVariables.song_list.sort((s2, s1) => {
  ...
  // 计算各种筛选条件
  // 状态
  window.gVariables.filter_options.status.push("--");
  window.gVariables.filter_options.status.push(
    ...new Set(window.gVariables.song_list.map((i) => i.status))
  );
  // 语言
  window.gVariables.filter_options.language.push("--");
  window.gVariables.filter_options.language.push(
    ...new Set(window.gVariables.song_list.map((i) => i.language))
  );
  // 演唱者
  let artist = new Set(["--"]);
  for (let song of window.gVariables.song_list)
    for (let a of song.artist.split(",")) artist.add(a);
  window.gVariables.filter_options.artist.push(...artist);
  // 月份
  window.gVariables.filter_options.month.push("--");
  window.gVariables.filter_options.month.push(
    ...new Set(window.gVariables.song_list.map((i) => i.date.substring(0, 7)))
  );
}
...
function convert_song(row) {
  ...
  if (days_before_available > 0 && !window.gVariables.backdoor)
    have_audio = false;
...
function parse_record(date, origin, record_index, record_start) {
  ...
  // 找bv号
  let recordings = window.gVariables.recording_list.filter(
    (r) => r.date === date && r.vup === origin
  );
  //
  return {

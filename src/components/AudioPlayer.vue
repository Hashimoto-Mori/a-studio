      ...
      <div class="c-playlist" v-show="show_playlist">
        ...
        <div class="c-playlist-songList" ref="playlist">
          <div
            ...
            v-bind:class="[
              'c-playlist-song',
              { 'c-playlist-playing': song.id === playlist[current_song].id },
            ]"
            v-bind:key="song.id"
...
export default {
  ...
  methods: {
    ...
    playlist_remove_song(idx) {
      ...
      // 重新获取当前歌index
      this.current_song = this.playlist.findIndex((song) => {
        return song.id === current_song_id;
      });
      // 保存当前歌单
      utils.save_playlist(this.current_song, this.playlist);
    },
    playlist_add_song(song, jump = false, auto_play = false) {
      ...
      // 判断歌曲是否重复
      var idx = this.playlist.findIndex((s) => {
        return s.id === song.id;
      });
      if (idx === -1) {
        // 如果不重复
  ...
  mounted() {
    ...
    audio.addEventListener('timeupdate', () => {
      if (audio.duration)
        this.play_progress = audio.currentTime / audio.duration;
